...
withEventDeliveryModes(
  ...
  async ({ page }) => {
    ...
    let langinfraCount = await page
      .locator('[data-testid="langinfra-0-option"]')
      .count();

    while (langinfraCount === 0) {
      ...
      langinfraCount = await page
        .locator('[data-testid="langinfra-0-option"]')
        .count();
    }

    await page.locator('[data-testid="langinfra-0-option"]').nth(0).waitFor({
      timeout: 15000,
      state: "visible",
    });

    await page.getByTestId("langinfra-0-option").nth(0).click();
