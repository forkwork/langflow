...
import { addFlowToTestOnEmptyLanginfra } from "../../utils/add-flow-to-test-on-empty-langinfra";
...
test(
  ...
  async ({ page }) => {
    ...
    const firstRunLanginfra = await page
      .getByTestId("empty-folder-description")
      .count();

    if (firstRunLanginfra > 0) {
      await addFlowToTestOnEmptyLanginfra(page);
    }
