...
from langinfra.services.deps import get_db_service, session_scope
...
@pytest.fixture
def runner(tmp_path):
    env = {"LANGINFRA_DATABASE_URL": f"sqlite:///{tmp_path}/test.db"}
...
async def get_starter_project(client, active_user):  # noqa: ARG001
    ...
    async with session_scope() as session:
        stmt = (
            select(Flow).where(Flow.folder.has(Folder.name == STARTER_FOLDER_NAME)).where(Flow.name == "Basic Prompting")
        )
