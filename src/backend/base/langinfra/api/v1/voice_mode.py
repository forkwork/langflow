...
from langinfra.api.v1.chat import build_flow_and_stream
...
from langinfra.services.auth.utils import api_key_header, api_key_query, api_key_security, get_current_user_by_jwt
from langinfra.services.database.models import MessageTable
from langinfra.services.database.models.flow.model import Flow
from langinfra.services.deps import get_variable_service, session_scope
from langinfra.utils.voice_utils import (
    BYTES_PER_24K_FRAME,
    VAD_SAMPLE_RATE_16K,
    resample_24k_to_16k,
)
            ...
            await client_websocket.send_json(
                {
                    "type": "error",
                    "code": "langinfra_auth",
                    "message": "You must pass a valid Langinfra token or cookie.",
                }
            )
            return None, None
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                log_event(msg, "↑")
                                num_audio_samples = 0
                        elif msg.get("type") == "langinfra.elevenlabs.config":
                            logger.info(f"langinfra.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            await openai_ws.send(message_text)
                        elif event.get("type") == "langinfra.elevenlabs.config":
                            logger.info(f"langinfra.elevenlabs.config {event}")
