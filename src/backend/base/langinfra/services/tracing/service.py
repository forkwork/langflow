...
def _get_opik_tracer():
    from langinfra.services.tracing.opik import OpikTracer
...
class TracingService(Service):
    ...
    async def start_tracers(
        ...
        if self.deactivated:
            return
        try:
            project_name = project_name or os.getenv("LANGCHAIN_PROJECT", "Langinfra")
    ...
    def project_name(self):
        if self.deactivated:
            return os.getenv("LANGCHAIN_PROJECT", "Langinfra")
        trace_context = trace_context_var.get()
        if trace_context is None:
