...
from langinfra.interface.components import get_and_cache_all_types_dict
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            logger.debug(f"Starting Langinfra v{version}...")
        else:
            logger.debug("Starting Langinfra...")
        ...
        try:
            ...
            # Final message
            logger.debug("Langinfra shutdown complete")
...
def create_app():
    ...
    if settings.mcp_server_enabled:
        from langinfra.api.v1 import mcp_router
