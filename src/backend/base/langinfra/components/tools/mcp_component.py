...
from langinfra.base.mcp.util import (
...
from langinfra.inputs import DropdownInput
from langinfra.inputs.inputs import InputTypes
from langinfra.io import MessageTextInput, Output, TabInput
from langinfra.io.schema import schema_to_langinfra_inputs
...
class MCPToolsComponent(Component):
    ...
    async def find_langinfra_instance(self) -> tuple[bool, int | None, str]:
        """Find Langinfra instance by checking env variable first, then scanning common ports."""
        # First check environment variable
        env_port = os.getenv("LANGINFRA_PORT")
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code < HTTP_ERROR_STATUS_CODE:
                    return True, port, f"Langinfra instance found at configured port {port}"
        except (ValueError, httpx.TimeoutException, httpx.NetworkError, httpx.HTTPError):
            logger.warning(f"Could not connect to Langinfra at configured port {env_port}")
        return False, None, "No Langinfra instance found on configured port or common ports"
    ...
    async def _validate_schema_inputs(self, tool_obj) -> list[InputTypes]:
        ...
        try:
            ...
            schema_inputs = schema_to_langinfra_inputs(input_schema)
        ...
        try:
            if field_name == "mode":
                ...
                if field_value == "Stdio":
                    ...
                    _, port, _ = await self.find_langinfra_instance()
                    if port:
            ...
            if field_name in ("command", "sse_url", "mode"):
                try:
                    ...
                    if field_name == "sse_url":
                        ...
                        if not is_valid:
                            found, port, message = await self.find_langinfra_instance()
                            if found:
    ...
    def get_inputs_for_all_tools(self, tools: list) -> dict:
        ...
        for tool in tools:
            if not tool or not hasattr(tool, "name"):
                continue
            try:
                input_schema = schema_to_langinfra_inputs(create_input_schema_from_json_schema(tool.inputSchema))
        ...
        try:
            ...
            if self.mode == "Stdio":
                ...
                try:
                    ...
                    msg = f"Invalid SSE URL configuration: {e}. Please check your Langinfra deployment URL and port."
                    raise ValueError(msg) from e
                except ConnectionError as e:
                    # Connection failed after retries
                    logger.error(f"SSE connection error: {e}")
                    msg = (
                        f"Could not connect to Langinfra SSE endpoint: {e}. "
                        "Please verify:\n"
                        "1. Langinfra server is running\n"
                        "2. The SSE URL matches your Langinfra deployment port\n"
