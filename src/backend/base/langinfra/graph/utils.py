...
from langinfra.serialization import serialize
...
async def log_transaction(flow_id: str | UUID, source: Vertex, status, target: Vertex | None = None, error=None) -> None:
    try:
...
def has_chat_output(vertices: dict[Vertex, int]):
    from langinfra.graph.schema import InterfaceComponentTypes
