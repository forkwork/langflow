---
title:  Integrate NVIDIA Ingest with Langinfra
...
* The **NVIDIA Ingest** component requires the installation of additional dependencies to your Langinfra environment. To install the dependencies in a virtual environment, run the following commands.

  * If you have the Langinfra repository cloned and installed from source:
  ```bash
  ...
  * If you are installing Langinfra from the Python Package Index:
  ```bash
  source **YOUR_LANGINFRA_VENV**/bin/activate
  uv pip install --prerelease=allow 'langinfra[nv-ingest]'
  uv run langinfra run
  ```
